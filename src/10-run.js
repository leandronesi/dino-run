...
(function(){'use strict';
  ...
  function reset(){S={phase:'ready',lane:1,x:1,lives:3,score:0,distance:0,rows:[],timer:.52,jump:0,duck:0,shield:0,passed:0,campAt:12,target:0,reason:'',elapsed:0,speed:G.level===1?.40:.52,combo:0,heartFlash:0};}
  function start(){
    S.phase='run';
    ...
    G.sfx('win');G.say('Via! Raccogli i frutti e cambia strada.');
  }
  function action(a){
  ...
  function update(dt){
    ...
    for(var i=S.rows.length-1;i>=0;i--){
      ...
      if(!row.hit&&row.z>=.93){
        ...
        if(good){S.combo++;S.score+=(kind==='fruit'?3:1)+(S.combo>=5?1:0);G.sfx('coin');}
